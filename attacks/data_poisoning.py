...
class TokenReplacedDataset(Dataset):
    ...
    For image datasets, this randomly replaces a fraction of pixel patches with
    adversarial patterns (shuffled patches from other samples), simulating the
    effect of domain-specific token replacement in NLP tasks applied to any modality.
    """
    ...
    def __getitem__(self, idx):
        data, label = self.dataset[idx]
        if self.rng.random() < self.replacement_fraction:
            # Replace a random region of the input with content from another sample
            donor_idx = self.rng.integers(0, self.n)
            donor_data, _ = self.dataset[donor_idx]
            # Create a mask covering a random rectangular region (~25% of area)
            if data.dim() >= 2:
                h, w = data.shape[-2], data.shape[-1]
                rh = max(1, h // 4)
                rw = max(1, w // 4)
                y0 = self.rng.integers(0, max(1, h - rh))
                x0 = self.rng.integers(0, max(1, w - rw))
                data = data.clone()
                data[..., y0:y0 + rh, x0:x0 + rw] = donor_data[..., y0:y0 + rh, x0:x0 + rw]
            else:
                # 1D data: replace a contiguous segment
                n_features = data.shape[0]
                seg_len = max(1, n_features // 4)
                start = self.rng.integers(0, max(1, n_features - seg_len))
                data = data.clone()
