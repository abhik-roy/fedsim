...
class TokenReplacedDataset(Dataset):
    ...
    def __init__(self, dataset, replacement_fraction: float = 0.3,
                 attack_fraction: float = 1.0, seed: int = 42):
        self.dataset = dataset
        self.replacement_fraction = replacement_fraction
        self.n = len(dataset)
        # Pre-select attacked indices and donor indices for determinism
        rng = np.random.default_rng(seed)
        num_attacked = max(1, int(self.n * attack_fraction))
        self.attacked_indices = set(rng.choice(self.n, size=num_attacked, replace=False))
...
def apply_token_replacement(dataset, replacement_fraction: float = 0.3,
                            attack_fraction: float = 1.0, seed: int = 42):
    """Apply token replacement attack to a dataset partition."""
    return TokenReplacedDataset(dataset, replacement_fraction, attack_fraction, seed)
