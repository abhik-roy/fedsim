...
class TrimmedMean(FedAvg):
    ...
    def aggregate_fit(
        ...
        if trim_count == 0 and self.beta > 0:
            import warnings
            warnings.warn(
                f"TrimmedMean: beta={self.beta} with {num_clients} clients yields "
                f"trim_count=0 (no trimming). Need beta >= {1/num_clients:.2f} "
                f"to trim at least 1 client per end.",
                stacklevel=2,
            )
        ...
        for layer_idx in range(len(weights[0])):
            ...
            aggregated.append(np.mean(trimmed, axis=0).astype(weights[0][layer_idx].dtype))
