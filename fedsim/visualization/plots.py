...
def plot_client_grid(grid_data: list[list[str]], num_clients: int, num_rounds: int,
    ...
    from visualization import COLOR_BORDER, COLOR_EXCLUDED as _COLOR_EXCLUDED, COLOR_FALSE_POS as _COLOR_FALSE_POS
    status_to_num = {
    ...
    # Uniform 1/6 bands for each of the 6 states (zmin=0, zmax=5).
    # State n maps to normalized value n/5. Each band spans exactly 1/6 of [0,1].
    _e = 0.001  # epsilon for sharp step transitions
    colorscale = [
        [0.0, COLOR_BENIGN],                       # 0 = benign
        [1/6 - _e, COLOR_BENIGN],
        [1/6, COLOR_ATTACKED],                     # 1 = attacked
        [2/6 - _e, COLOR_ATTACKED],
        [2/6, COLOR_MALICIOUS_IDLE],               # 2 = malicious idle
        [3/6 - _e, COLOR_MALICIOUS_IDLE],
        [3/6, COLOR_EMPTY],                        # 3 = idle / not selected
        [4/6 - _e, COLOR_EMPTY],
        [4/6, _COLOR_EXCLUDED],                    # 4 = excluded (TP)
        [5/6 - _e, _COLOR_EXCLUDED],
        [5/6, _COLOR_FALSE_POS],                   # 5 = false positive (FP)
