...
def plot_accuracy_surface(results, num_rounds: int, metric_name: str = "Accuracy") -> go.Figure:
    ...
    # Use a single x-offset across all strategies so mixed-strategy traces align.
    # Non-"Accuracy" metrics come from eval/* custom metrics which start at round 1;
    # "Accuracy" uses round_accuracies which includes round 0 (pre-training).
    global_x_offset = 1 if metric_name != "Accuracy" else 0
    for i, r in enumerate(results):
        ...
        if metric_name != "Accuracy" and r.custom_metrics:
            for mk, vals in r.custom_metrics.items():
                ...
                # Strategy lacks the custom metric — drop round 0 to align with round 1+.
                y_values = y_values[1:] if len(y_values) > 0 else y_values
        rounds = list(range(global_x_offset, global_x_offset + len(y_values)))
