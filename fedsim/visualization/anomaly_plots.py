...
def plot_removal_f1_over_rounds(anomaly_history, num_rounds):
    ...
    # Tolerate partial/legacy history entries without P/R/F1 keys.
    _nan = float("nan")
    precisions = [r.get("precision", _nan) for r in anomaly_history]
    recalls = [r.get("recall", _nan) for r in anomaly_history]
    f1s = [r.get("f1", _nan) for r in anomaly_history]
...
def plot_exclusion_timeline(anomaly_history, num_clients, num_rounds, malicious_clients):
    ...
    # Uniform epsilon at every band boundary for sharp step transitions.
    _e = 0.001
    colorscale = [
        [0.0, _COLOR_TN], [1/3 - _e, _COLOR_TN],       # TN dark blue-gray
        [1/3, _COLOR_TP], [2/3 - _e, _COLOR_TP],       # TP deeper mint
        [2/3, _COLOR_FP], [1.0 - _e, _COLOR_FP],       # FP warm sand
        [1.0, _COLOR_FN],                              # FN muted coral (endpoint)
    ]
    z_normalized = z / 3.0
...
def plot_confusion_summary(anomaly_summary):
    ...
    # Discrete 4-value z-matrix: each cell gets its own color.
    # Rows bottom-to-top (Plotly default): row 0=Benign, row 1=Malicious
    # Columns: Excluded, Included
    # z values: FP=0, TN=1, TP=2, FN=3
    z_color = [[0, 1], [2, 3]]
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        colorscale=[
            [0, _COLOR_FP], [0.33, _COLOR_FP],       # FP = sand
            [0.33, _COLOR_TN], [0.5, _COLOR_TN],     # TN = neutral
            [0.5, _COLOR_TP], [0.83, _COLOR_TP],      # TP = mint
            [0.83, _COLOR_FN], [1.0, _COLOR_FN],      # FN = coral
        ],
