...
class _TransformOverrideSubset(torch.utils.data.Dataset):
    """Wraps a Subset so items use a different transform. Thread-safe via lock."""
...
def _max_gpu_concurrent(model, device, use_amp=False):
    ...
    per_client_with_overhead = int(per_client * 1.3)
    return max(1, available // per_client_with_overhead)
...
def _validate_config(config: SimulationConfig):
    ...
    if atk.attack_type != "none":
        ...
        if atk.attack_type == "gaussian_noise":
            ...
            # Negative SNR (noise > signal) is a legitimate high-intensity attack.
            # Only reject exact zero, which degenerates the noise-scaling math.
            if snr == 0:
                raise ValueError(f"SNR must be non-zero (dB), got {snr}")
            ...
            # Amplification attack: scale_factor <= 1 attenuates the update
            # (moves it toward global), inverting the attack's intent.
            if sf <= 1.0:
                raise ValueError(
                    f"Gradient scale_factor must be > 1.0 (amplification); "
                    f"got {sf}. Use values like 5-100 for meaningful attacks."
                )
...
def _validate_compatibility(config: SimulationConfig) -> tuple[list[str], list[str]]:
    ...
    # Validate strategy-specific client count requirements.
    # Sampling picks floor(num_clients * fraction_fit) clients per round, so Bulyan's
    # n>=4f+3 invariant must hold against the *effective* pool, not total clients.
    atk = config.attack
    num_malicious = int(config.num_clients * atk.malicious_fraction) if atk.attack_type != "none" else 0
    effective_clients = max(
        config.min_fit_clients,
        min(config.num_clients, int(config.num_clients * config.fraction_fit)),
    )
    for s in config.strategies:
        if s == "bulyan" and num_malicious > 0:
            required = 4 * num_malicious + 3
            if effective_clients < required:
                errors.append(
                    f"Bulyan requires n >= 4f+3 (effective n={effective_clients}, "
                    f"f={num_malicious}, need {required}). Increase num_clients, "
                    f"raise fraction_fit, or reduce malicious_fraction."
                )
...
def _get_strategy(name, initial_parameters, num_clients, num_malicious=0,
    ...
    if name == "fedavg":
        ...
        beta = (num_malicious / num_clients) if num_clients > 0 else 0.0
...
def _is_float_target(sample_target) -> bool:
    """Detect float-valued targets (regression datasets) to skip label flipping."""
    return (
        isinstance(sample_target, float)
        or isinstance(sample_target, np.floating)
        or (hasattr(sample_target, "is_floating_point") and sample_target.is_floating_point())
    )
...
def _compute_trust_scores(client_params_list, global_params):
    ...
    # Guard: if any client produced NaN params, return NaN scores
    if not np.all(np.isfinite(deltas_arr)):
        return {cid: float('nan') for cid in range(len(client_params_list))}
...
def _set_model_params(model, parameters):
    ...
    state_dict = OrderedDict(
        {k: v.clone().detach() if isinstance(v, torch.Tensor)
         else torch.from_numpy(np.asarray(v))
    ...
    if attack_category == "data":
        for cid in malicious_clients:
            ...
            if attack_cfg.attack_type == "label_flipping":
                ...
                if _is_float_target(sample_target):
                    import warnings
...
def _run_client_round(
    ...
    if trainloader is not None:
        pass  # Use provided trainloader
    elif is_malicious and attack_category == "data" and attack_active and cid in round_poisoned_loaders:
    ...
    compiled = False
    _compile_failed = compile_failed_event.is_set() if compile_failed_event else False
    if not _compile_failed and config.compile_model and hasattr(torch, "compile"):
        ...
        _set_model_params(client_model, global_params)
        # eval_model is reused across clients/rounds, so invalidate any plugin-cached
        # per-client state (e.g., FedProx's proximal anchor snapshot) before training
        # starts. Without this, plugins see stale weights from the previous client.
        _stale_attrs = [a for a in vars(client_model) if a.startswith("_fedprox_")]
        for _a in _stale_attrs:
            delattr(client_model, _a)
    ...
    try:
        ...
        # Only treat as a compile-related failure when it actually looks like one.
        # Otherwise (OOM, NaN, plugin errors) re-raise without permanently disabling
        # torch.compile via compile_failed_event.
        _is_compile_failure = False
        if compiled:
            _msg = str(e).lower()
            _is_compile_failure = (
                "compile" in _msg or "dynamo" in _msg or "inductor" in _msg
                or type(e).__module__.startswith("torch._dynamo")
            )
...
def _process_round_results(
    ...
    cid_list = selected_cids if selected_cids is not None else list(range(len(client_results)))
...
def _run_clients_parallel(
    ...
    model_plugin=None, strategy_plugin=None, compile_failed_event=None,
):
    ...
    def _train_one(cid):
        ...
        try:
            # Create per-thread DataLoader to avoid shared iterator state
            is_malicious = cid in malicious_clients
            attack_cat = setup["attack_category"]
            if is_malicious and attack_cat == "data" and attack_active and cid in round_poisoned_loaders:
                partition = round_poisoned_loaders[cid].dataset
            else:
                partition = clean_partitions[cid]
            # Collision-free per-(cid, round) seed: cid * (num_rounds+1) + rnd
            # guarantees no two (cid, rnd) pairs share a seed for any cid < num_clients.
            g = torch.Generator().manual_seed(
                config.seed + cid * (config.num_rounds + 1) + rnd
            )
            pin = config.pin_memory and torch.cuda.is_available()
            thread_loader = DataLoader(
                partition, batch_size=config.batch_size,
            ...
            with ctx:
                ...
                updated_params, client_loss, client_acc, status, attack_applied = (
                    _run_client_round(
                        ...
                        model_plugin=model_plugin,
                        strategy_plugin=strategy_plugin,
                        compile_failed_event=compile_failed_event,
                        trainloader=thread_loader,
                    )
                )
            if stream:
                stream.synchronize()
    ...
    if max_workers <= 1:
        ...
        _compile_failed = compile_failed_event.is_set() if compile_failed_event else False
        if config.compile_model and not _compile_failed:
            thread_eval_model = copy.deepcopy(template_model)
        else:
...
def run_simulation(
    ...
    for strat_idx, strategy_name in enumerate(config.strategies):
        # Reset per-strategy so a compile failure in one strategy doesn't
        # permanently disable compilation for subsequent strategies.
        compile_failed_event = threading.Event()
        if progress_callback:
            progress_callback(strategy_name, strat_idx, len(config.strategies))

        # Seed before model init so each strategy starts from identical weights
        ...
        initial_parameters = [p.copy() for p in global_params]
        ...
        for rnd in range(1, config.num_rounds + 1):
            ...
            # Accumulate trust scores — pad non-selected clients with NaN
            # so all clients have equal-length history for visualization
            for cid, score in trust_scores.items():
                trust_hist.setdefault(cid, []).append(score)
            for cid in all_cids:
                if cid not in trust_scores:
                    trust_hist.setdefault(cid, []).append(float('nan'))
            ...
            # Accumulate reputation scores — pad non-selected clients with NaN
            for cid, score in reputation_scores.items():
                rep_hist.setdefault(cid, []).append(score)
            if reputation_scores:  # only pad if strategy tracks reputation
                for cid in all_cids:
                    if cid not in reputation_scores:
                        rep_hist.setdefault(cid, []).append(float('nan'))
