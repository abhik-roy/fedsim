...
class FedAvg(Strategy):
    ...
    def aggregate_fit(
        ...
        for cid, fit_res in results:
            if all(np.all(np.isfinite(p)) for p in fit_res.parameters):
                clean.append((cid, fit_res))
        if len(clean) < len(results):
            warnings.warn(
                f"FedAvg: filtered {len(results) - len(clean)} clients with NaN/Inf parameters."
            )
        results = clean
        if not results:
            return None, {}
        ...
        orig_dtypes = [p.dtype for p in results[0][1].parameters]
        aggregated = [
        ...
        # Cast back to original dtypes to avoid silent float64 upcasting of model
        aggregated = [a.astype(dt) for a, dt in zip(aggregated, orig_dtypes)]
        return aggregated, {}
