...
def test_token_replacement_attack_fraction():
    """TokenReplacedDataset should support partial attack fraction."""
    from attacks.data_poisoning import TokenReplacedDataset
    ds = TensorDataset(torch.randn(100, 3, 8, 8), torch.zeros(100, dtype=torch.long))
    attacked = TokenReplacedDataset(ds, replacement_fraction=0.3, attack_fraction=0.5, seed=42)
    assert len(attacked.attacked_indices) == 50
