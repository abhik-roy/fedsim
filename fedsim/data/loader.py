...
def get_dataset(dataset_name: str, data_dir: str | None = None, **kwargs):
    ...
    if dataset_name == "mnist":
        ...
        train = torchvision.datasets.SVHN(data_dir, split="train", download=True, transform=train_transform)
        test = torchvision.datasets.SVHN(data_dir, split="test", download=True, transform=test_transform)

    elif dataset_name == "femnist":
        # EMNIST byclass stores images transposed vs standard MNIST —
        # apply transpose after ToTensor to correct orientation.
        transform = transforms.Compose([
            transforms.ToTensor(),
...
class _MedMNISTWrapper(torch.utils.data.Dataset):
